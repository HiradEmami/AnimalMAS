...
import animalsimulation.behavior.movement.WorkerBeeExploitMovementMove;
 ...
 * @author Ebombo2
 */
public class WorkerBeeBehaviorMove  extends StateMachine {
    
    private static final double KnowledgeUpdateProbability = 1.0d;
    
    public WorkerBeeBehaviorMove(Agent agent) {
        super(agent);
        
        State idle = new State("Idle",  new UpdateHiveStatistics());
        State communicate = new State("Communicate",  new UpdateHiveStatistics());
        State moveToFoodSource = new State("TravellingToFood", new WorkerBeeExploitMovementMove());
